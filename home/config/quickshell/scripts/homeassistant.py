...
def parse_args() -> argparse.Namespace:
    ...
    subparsers.add_parser("scenes", help="List configured scene entities.")
    ...
    scene_activate_parser = subparsers.add_parser("activate-scene", help="Activate a scene.")
    scene_activate_parser.add_argument("entity_id")

    return parser.parse_args()

def get_config() -> tuple[str, str, list[str], list[str]]:
    """Return (url, token, allowed_lights, allowed_scenes) from the repo env file."""
    ...
    raw_scene_allowlist = env.get("RBW_HA_SCENES", "")
    scene_allowlist = [part.strip() for part in raw_scene_allowlist.split(",") if part.strip()]
    if not scene_allowlist:
        scene_allowlist = list(DEFAULT_ALLOWED_SCENES)
    return hass_url, token, allowlist, scene_allowlist
...
def ensure_light_entity_id(entity_id: str, allowlist: list[str]) -> None:
    """Validate that the light entity ID is safe and optionally allowed."""
...
def ensure_scene_entity_id(entity_id: str, allowlist: list[str]) -> None:
    """Validate that the scene entity ID is safe and optionally allowed."""
    if not SCENE_ID_RE.match(entity_id):
        raise HomeAssistantError(f"Invalid scene entity_id: {entity_id!r}")
    if allowlist and entity_id not in allowlist:
        raise HomeAssistantError(f"Scene {entity_id!r} is not in RBW_HA_SCENES")
...
def normalize_scene(state: dict[str, Any]) -> dict[str, Any]:
    """Normalize a Home Assistant scene state object for QML."""
    entity_id = str(state.get("entity_id", ""))
    attributes = state.get("attributes", {})
    if not isinstance(attributes, dict):
        attributes = {}
...
def sort_scenes(scenes: list[dict[str, Any]], allowlist: list[str]) -> list[dict[str, Any]]:
    """Sort scenes by allowlist order or by friendly name."""
    if allowlist:
        order = {entity_id: index for index, entity_id in enumerate(allowlist)}
        return sorted(
            scenes,
            key=lambda scene: (
                order.get(str(scene.get("entityId", "")), len(order)),
                str(scene.get("name", "")).lower(),
            ),
        )
    ...
    return sorted(
        ...
        key=lambda scene: (
            str(scene.get("name", "")).lower(),
            str(scene.get("entityId", "")).lower(),
        ),
    )


def list_lights(
    hass_url: str, token: str, allowlist: list[str], scene_allowlist: list[str]
) -> dict[str, Any]:
    """Fetch Home Assistant state and return compact light+scene summary."""
    if not token:
        return {
            "configured": False,
            "available": False,
    ...
    scene_allowset = set(scene_allowlist)
    for state in states:
        ...
        if entity_id.startswith("scene."):
            if scene_allowlist and entity_id not in scene_allowset:
                continue
            scenes.append(normalize_scene(state))
            continue

    lights = sort_lights(lights, allowlist)
    scenes = sort_scenes(scenes, scene_allowlist)
...
def call_light_service(
    ...
    *,
    ...
    ensure_light_entity_id(entity_id, allowlist)
...
def call_scene_activation(
    ...
    *,
    ...
    """Activate a safe scene-domain action."""
    if not token:
        raise HomeAssistantError("Missing HA_TOKEN")
    ensure_scene_entity_id(entity_id, allowlist)
...
def main() -> int:
    """Run the CLI."""
    args = parse_args()
    hass_url, token, allowlist, scene_allowlist = get_config()

    try:
        if args.command == "lights" or args.command == "scenes":
            print_json(list_lights(hass_url, token, allowlist, scene_allowlist))
            return 0
